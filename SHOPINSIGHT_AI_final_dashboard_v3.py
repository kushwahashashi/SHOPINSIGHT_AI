...
section[data-testid="stSidebar"] label { padding: 7px 10px; border-radius: 9px; }
...
# -----------------------------
st.markdown(
    '<div style="background:white;border:1px solid #e1e8f6;border-radius:12px;padding:9px 14px;margin-bottom:10px;color:#60759d;font-size:12px;box-shadow:0 4px 14px rgba(25,54,105,.05);">'
    '🔎 <b style="color:#234783;">Search insight:</b> Products • Customers • Orders • Sales • Reviews'
    '<span style="float:right;color:#7890b8;">📅 Olist dataset • Executive view</span>'
    '</div>', unsafe_allow_html=True
)
profile_path = find_file("profile_photo.jpg")

c1, c2 = st.columns([4.15, 1.65])
...
with c2:
    ...
    st.markdown(
        f"""
        <div class="profile-card" style="text-align:center;padding:12px 10px;min-height:154px;">
            {photo_html}
...
if nav == "Dashboard":
    ...
    st.markdown('<div style="height:1px;background:#dfe7f5;margin:18px 0 8px;"></div>', unsafe_allow_html=True)
    st.markdown('<div class="section-title">📈 Sales Performance</div>', unsafe_allow_html=True)
    a, b = st.columns(2)

    with a:
        st.markdown("#### 🛒 Monthly Orders")
        st.line_chart(monthly.set_index("month")["orders"], height=285)

    with b:
        st.markdown("#### 💰 Monthly Revenue")
        st.line_chart(monthly.set_index("month")["revenue"], height=285)
